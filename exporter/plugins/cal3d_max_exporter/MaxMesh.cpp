...
//  m_pIMesh->buildNormals(); // forced normal building <- this call corrupts normals in 3dsmax9
  ...
  if(m_pModifier == 0)
  {
    ...
    {
		m_modifierType = MODIFIER_NONE;
/*		//AfxMessageBox((std::string("No physique/skin modifier found. ") + pINode->GetName()).c_str());
      ...
      return false;*/
		return true; // we now support unskinned meshes
    }
    else
    {
...
#endif
//   else // we now support unskinned meshes
//   {
...
//     }
  ...
  RVertex *pRVertex;
  pRVertex = m_pIMesh->getRVertPtr(vertexId);// =NULL???
  if ( pRVertex == NULL )
  {
	  AfxMessageBox("pRVertex == NULL", MB_OK | MB_ICONEXCLAMATION);
	  return Point3( 0, 0, 0 );
  }
